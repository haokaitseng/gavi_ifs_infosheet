...
dat_cofi_paid_nonFY <- read_excel("41. Analytics/cofi-trends/Infosheet co-financing history/output_table_infosheet/infosheet_payment_history_2024-11-30.xlsx", sheet="Non-FY countries")|>  clean_names() 
...
#| fig.width: 8
#| warning: false
# dat_cofi_paid_nonFY <- read_excel("data/infosheet_payment_history_2024-11-30.xlsx", sheet="Non-FY countries")|>  clean_names() 

# prepare temporary dataset by vaccines
temp <- dat_cofi_paid_nonFY |> 
  filter(iso3 == params$current_iso3 )|>
  mutate(vaccine_group_sel = case_when( 
...
temp <- dat_projection|>
  filter(iso3 == params$current_iso3,
         vaccine_group!="IPV")|> # to drop the level of IPV
  left_join(dat_vaccine_groups|> 
...
# total_cofi_usd <- temp %>%
#   group_by(year) %>%
#   summarise(total = sum(cofi_usd, na.rm = TRUE))
# 
# max_total <- max(total_cofi_usd$total, na.rm = TRUE)
# # Define the y-axis scale based on the condition
# y_scale <- if (max_total > 1e6) {
#   scale_y_continuous(labels = scales::label_number(scale = 1e-6, suffix = "")
#                      #,breaks = seq(0, max(temp$cofi_usd, na.rm = TRUE), by = 5e6)
#                      )
# } else {
#   scale_y_continuous(labels = scales::label_number(scale = 1e-3, suffix = "")
#                      #,breaks = seq(0, max(temp$cofi_usd, na.rm = TRUE), by = 5e3)
#                      )
# }
...
ggplot(temp) + 
  ...
  labs(x = "", 
       ...
       fill = "")+
  scale_fill_manual(values = as.vector(gavi_colors(levels(temp$vaccine_group_gavir))))+
  scale_y_continuous(labels = scales::label_number(scale = 1e-6, suffix = "")
                     #,breaks = seq(0, max(dat_projection$cofi_usd, na.rm = TRUE), by = 1e6)
                     )

#can't use temp df due to lack of ipv
dat_projection |>
  filter(iso3 == params$current_iso3)|>
...
#translation
  ...
  select(vaccine_group_sel, 
         all_of(as.character(params$current_year : params$projected_year)))|> #`2024`:`2029`
...
vaccines_fr_future <- temp |> # use temp because fr vaccine names are merged already
  filter(iso3 == params$current_iso3) |>
  mutate(current_year = year == params$current_year) |>
...
temp <- dat_projection |>
  ...
  temp|>
    ...
    rename_with(~ if_else(params$lang == "fr" & . == "Total", 
                          "Totale", . ))|>
#table stlying:    
  tt(theme = "striped", width= c(1.5,rep(1,6)) )|>
  style_tt( 
    i = 0, #header
    color = "white", background = "#005CB9", align = "c")|> 
  style_tt(
